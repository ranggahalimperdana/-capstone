...
export default function Login({ onClose, onLoginSuccess, onSwitchToRegister, onForgotPassword }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Store role info and redirect based on role
    
    setTimeout(() => {
      // Save user data temporarily for App.tsx to pick up
      const savedUsers = localStorage.getItem('uninotes_users');
      const users = savedUsers ? JSON.parse(savedUsers) : {};
      
      // Check if user exists
      if (users[email]) {
        // Ensure user has a role (default to 'user' for backward compatibility)
        if (!users[email].role) {
          users[email].role = 'user';
        }
        
        localStorage.setItem('uninotes_temp_login', JSON.stringify(users[email]));
        setIsLoading(false);
        onLoginSuccess();
        onClose();
      } else {
